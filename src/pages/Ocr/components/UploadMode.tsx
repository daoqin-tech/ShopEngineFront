...
export function UploadMode({ projectId }: UploadModeProps) {
  ...
  const uploadAndRecognize = async (image: UploadedImage) => {
    try {
      ...
      const objectKey = `ocr-images/${projectId}/${Date.now()}-${image.file.name}`
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex-1 px-8 py-12 max-w-4xl mx-auto space-y-8">
        {/* 图片展示区域 */}
