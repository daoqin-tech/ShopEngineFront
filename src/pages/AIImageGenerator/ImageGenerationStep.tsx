...
export function ImageGenerationStep({ 
  ...
  const generatedImages = Array.from(session.images?.values() || []).filter(img => img.status === 'completed');
          ...
          <div className="max-h-[calc(100vh-350px)] overflow-y-auto">
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
              {generatedImages.map((image, index) => {
                const prompt = session.prompts.get(image.promptId);
                return (
                  <div key={image.id} className="group relative space-y-3 p-4 border rounded-lg bg-gray-50 hover:shadow-lg transition-all duration-200">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-gray-400">
                          {image.metadata?.width || 1024}×{image.metadata?.height || 1024}
                        </span>
                      </div>
