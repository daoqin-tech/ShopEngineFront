...
import { MessageSquare, CheckCircle, Edit3, ArrowRight, Check, Square } from 'lucide-react';
                        ...
                        {msg.prompts && msg.prompts.length > 0 && (
                          <div className="mt-3 space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              {(() => {
                                const allSelected = msg.prompts?.every(p => session.prompts?.find(sp => sp.id === p.id)?.selected);
                                return (
                                  <button
                                    onClick={() => {
                                      msg.prompts?.forEach(prompt => {
                                        const isCurrentlySelected = session.prompts?.find(p => p.id === prompt.id)?.selected;
